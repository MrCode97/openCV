...
while(1):                                                                       # endless loop
    ...
    if ret == True:                                                             # check if there is still incoming frame
        ...
        # Draw it on image
        x,y,w,h = track_window                                                  # get coordinates from track_window
        img2 = cv2.rectangle(frame, (x,y), (x+w,y+h), 255, 2)                   # draw rectangle from track_window-coordinates
        cv2.imshow('img2',img2)
        k = cv2.waitKey(60) & 0xff                                              # listen and store key
        if k == 27:
            break
        else:
            cv2.imwrite(chr(k)+".jpg",img2)                                     # write img with name from stored key
    else:
        break
